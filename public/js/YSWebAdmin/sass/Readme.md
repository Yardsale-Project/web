...
folder are mapped to the application's root namespace, 'YSWebAdmin'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "YSWebAdmin"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'YSWebAdmin' in the same way
as `"YSWebAdmin/sass/src"`.
